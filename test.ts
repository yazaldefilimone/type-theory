...
function max(m: number, n: number): number {
  if (m === 0) {
      return n;
  } else {
      return max(m - 1, n > 0 ? n - 1 : n);
  }
}

// Exemplo de uso
console.log(max(5, 10)); // Saída: 10
console.log(max(0, 7)); // Saída: 7
console.log(max(3, 3)); // Saída: 3
console.log(max(10, 5)); // Saída: 10